...
void MOXExport::UpdateOptions(HWND hWnd, bool bSave)
{
	...
	{
		MessageBox(hPanel, "'Put Into One' ͬ 'Force Animation' ��ͻ!\n���Զ�ȥ��ѡ��'Force Animation'!", "ѡ���ͻ", MB_OK);
...
bool MOXExport::IGameSaveSelected(HWND hWnd, char * szFileName, MOX_OPTION * pOption, bool bBinary)
{
	...
	if( nNumNode == 0)
	{
		MessageBox(NULL, "��ѡȡģ�ͽڵ㣡", "ERROR", MB_ICONERROR);
		pIgame->ReleaseIGame();
		delete pScene;
		pScene = NULL;
		return false;
...
bool MOXExport::IGameAddSubMeshToFrame(A3DFrame* pFrame, IGameNode* pGameNode, IGameMesh * pGameMesh, IGameMaterial* pSubMaterial, MOX_OPTION * pOption)
{
	...
	if (idLM != 2)
	{
		//MessageBox(NULL, "ע��û������lightmap uv!", pGameNode->GetMaxNode()->GetName(), MB_ICONWARNING);
		bHasLightMap = false;
		//return true;
	}
	...
	{
		MessageBox(NULL, "ע�⣡�е�sub mesh û�ж��㣡ϵͳ���Զ����˵���mesh!", pGameNode->GetMaxNode()->GetName(), MB_ICONSTOP);
		return true;
	}
	...
	{
		MessageBox(NULL, "�����Ż�ʧ�� [%s] !", pGameNode->GetMaxNode()->GetName(), MB_ICONSTOP);
	}
	...
	if (pNode->GetUserPropBool("˫��", bval) && bval)
		prop.Set2SidesFlag(true);
	if (pNode->GetUserPropBool("���䴩", bval) && bval)
		prop.SetShootThroughFlag(true);
	if (pNode->GetUserPropBool("������", bval) && bval)
		prop.SetNoMarkFlag(true);
	if (pNode->GetUserPropBool("������", bval) && bval)
		prop.SetPickThroughFlag(true);
	if (pNode->GetUserPropBool("�ر���", bval) && bval)
		prop.SetOnGroundFlag(true);
	if (pNode->GetUserPropBool("�ָ���", bval) && bval)
		prop.SetBSPPVSFlag(true);
	if (pNode->GetUserPropBool("����Ⱦ", bval) && bval)
	...
	if( !bHasLightMap) //��meshû��light map uv
		pLM = NULL;
...
bool MOXExport::AddNodeToFrame(HWND hD3DWnd,IDirect3DDevice8 * pDevice, A3DFrame * pParentFrame, INode * pNode, INode * pParentNode, bool bAnimatedParent, MOX_OPTION * pOption)
{
	...
	if( pNode->Selected() && (!pNode->IsGroupHead()) && os.obj->ClassID() == Class_ID(DUMMY_CLASS_ID, 0) )
	{
		...
		if( strstr(szName, "HH_") == szName )
		{
			...
			{
				char szInfo[256];
				sprintf(szInfo, "Ҫ����������λ��(%s)�������ӵ�ĳ�������ϣ�", szName);
				MessageBox(hD3DWnd, szInfo, "ʧ��", MB_OK);
				return false;
			}
		...
		else if( strstr(szName, "BB_") == szName )
		{
			...
			{
				char szInfo[256];
				sprintf(szInfo, "Ҫ�����İ�Χ��(%s)�������ӵ�ĳ�������ϣ�", szName);
				MessageBox(hD3DWnd, szInfo, "ʧ��", MB_OK);
				return false;
			}
		...
		{
			char szInfo[256];
			sprintf(szInfo, "�������ǰ�Χ�е�Dummy����(%s)�����", szName);
			MessageBox(hD3DWnd, szInfo, "����", MB_OK);
			return true;
		}
		...
		{
			char szInfo[256];
			sprintf(szInfo, "��Ϊ��Χ�е����� %s �Ѿ����ǳ������ˣ������˲��б��Ρ�����!", pNode->GetName());
			MessageBox(hD3DWnd, szInfo, "ʧ��", MB_ICONSTOP | MB_OK);
			return false;
		}
		...
		if (pNode->GetUserPropBool("���ɴ�", bval) && bval)
		...
		{
			MessageBox(hD3DWnd, "�����Χ��ʧ�ܣ�", "ʧ��", MB_OK);
			return false;
		}
